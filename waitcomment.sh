...
p_track="`printf '|%s' "${track[@]}" | sed 's/|/|COMMAND=\/usr\/bin\//g' | \
tail -c +2`"
p_exclude="`printf '|%s' "${exclude[@]}" |  sed 's/|/|COMMAND=\/usr\/bin\//g' | \
tail -c +2`"
...
if grep -qE "$p_track|>" <<< "$curr_command"; then
	...
	if grep -qE -v "$p_exclude|$tmpdir" <<< "$curr_command"; then
		...
		do 
  		...
  		# addcomment runs (normally) as user so these files need to be
		# world-writable.
